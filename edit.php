			  	...
			  	<form method="POST" class="fix_form">
					...
					while($row = mysqli_fetch_assoc($result)):
				 	...
				 	?>	
			  		...
			  		<input type="text" name="anh" value="<?php echo $row['avatar'] ?>" class="fix_input">
			  		...
			  		<select class="fix_input" name="nganh">
			  			<?php 
			  				...
			  				$result_dp = mysqli_query($connect,"SELECT * FROM department WHERE idDepartment='$value' ");
			  				while ($row1 = mysqli_fetch_assoc($result_dp)):
			  					
			  				
			  			 ?>
			  			<option selected  value="<?php echo $row1['idDepartment'] ?>"> <?php echo $row1['nameDepartment'] ?></option>
			  				<?php endwhile; ?>
